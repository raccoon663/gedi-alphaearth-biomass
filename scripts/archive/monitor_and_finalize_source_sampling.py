...
def main() -> None:
    ...
    while True:
        ...
        for record in failed_audits:
            ...
            result = subprocess.run(
                [sys.executable, str(root / "scripts" / "01_data_extraction" / "submit_source_sampling_audit_recovery.py"),
    ...
    for year, kinds in sorted(recovered_by_year.items()):
        result = subprocess.run(
            [sys.executable, str(root / "scripts" / "archive" / "materialize_source_sampling_audit_recovery.py"),
    ...
    result = subprocess.run(
        [sys.executable, str(root / "scripts" / "02_data_preparation" / "finalize_source_sample.py"), str(drive)],
        cwd=root, capture_output=True, text=True, encoding="utf-8")
    if result.returncode != 0:
        write_log(completion_path, {
